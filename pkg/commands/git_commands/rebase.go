...
func (self *RebaseCommands) EditCommitAuthor(commits []*models.Commit, index int, author string) error {
	if index == 0 {
		// we've selected the top commit so no rebase is required
		return self.commit.AmendAuthor(author)
	}
	...
	err = self.commit.AmendAuthor(author)
	if err != nil {
		return err
	}
